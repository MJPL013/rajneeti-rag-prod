...
class RAGOrchestrator:
    """
    Central orchestrator. Accepts optional llm_provider / api_key / model_name at init time
    ...
    @staticmethod
    def _init_generator(llm_provider=None, api_key=None, model_name=None):
        from src.rag.generator import Generator
        return Generator(llm_provider=llm_provider, api_key=api_key, model_name=model_name)
    ...
    def query(self, query: str, filters: Dict[str, Any] = None, engine: Optional[str] = None) -> Dict[str, Any]:
        """Execute a RAG query. Routes to graph or vector engine based on argument."""
        use_graph = False
    ...
    def _intent_pipeline(self, query: str, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Intent: {analysis.intent.value} | Politician: {analysis.politician} | Topics: {analysis.topic_keywords}")

        grounded_ids = self._graph_engine.vector_ground(query, top_k=settings.TOP_K_RETRIEVAL)
    ...
    def _execute_traversal(self, analysis: QueryAnalysis, grounded_ids: List[str], query: str, filters: Dict[str, Any] = None) -> RAGContext:
