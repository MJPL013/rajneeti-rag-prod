...
class GraphRAGEngine:
    ...
    def vector_ground(self, query: str, top_k: int = 20) -> List[str]:
        """
        Embed the query, hit the Neo4j vector index, return grounded statement_ids.
        """
    ...
    def get_temporal_evolution(self, politician: str, statement_ids: List[str]) -> RAGContext:
        cypher = """
        ...
        try:
            with self.driver.session(database=settings.NEO4J_DATABASE) as session:
                ...
                for record in result:
                    ...
                    timeline.append(TimelineEntry(year_month=ym, statements=stmts, sources=srcs))
    ...
    def get_media_portrayal(self, politician: str, statement_ids: List[str]) -> RAGContext:
        cypher = """
        MATCH (p:Politician {name: $politician})<-[:FOCUSES_ON]-(a:Article)
        ...
        try:
            with self.driver.session(database=settings.NEO4J_DATABASE) as session:
                ...
                for record in result:
                    ...
                    stmts = record["statements"] or []
                    media_groups.append(MediaGroup(leaning=leaning, source_names=src_names, statements=stmts))
    ...
    def get_politician_persona(self, politician: str, statement_ids: List[str]) -> RAGContext:
        cypher = """
        ...
        if not rhetoric and not high_weight:
            rhetoric, high_weight, all_sources = self._persona_fallback(politician, statement_ids)
    ...
    def get_factual_context(self, query: str, politician: Optional[str] = None, filters: Dict[str, Any] = None) -> RAGContext:
    ...
    def _persona_fallback(self, politician: str, statement_ids: List[str]):
        cypher = """
        MATCH (p:Politician {name: $politician})<-[:FOCUSES_ON]-(a:Article)
              -[:CONTAINS_STATEMENT]->(s:Statement)
        WHERE s.statement_id IN $sids
    ...
    def _vector_search_rerank(self, query: str, filters: Dict[str, Any] = None) -> List[Dict]:
        if not self.is_connected():
            raise GraphDBError("Neo4j driver is not connected.")
